...
from app.services.data.project_service import ensure_default_project, get_active_project
from app.services.data.cached_queries_service import (
...
# Import tool calling system
from app.services.tools.agent import ToolCallingAgent
from app.services.tools.registry import tool_registry
# Import tools to register them
import app.services.tools.employee_tools  # noqa: F401
import app.services.tools.aggregation_tools  # noqa: F401
import app.services.tools.flexible_query  # noqa: F401
...
Provide a helpful response using the company and workforce data available."""
    ...
    async def chat_with_tools(
        ...
        """
        Chat using the tool-calling agent for data analysis questions.

        This method uses the ToolCallingAgent to dynamically call data analysis
        tools based on the user's question. The LLM decides which tools to call
        and synthesizes the results into a final answer.

        Use this for questions like:
        - "How many employees in Engineering?"
        - "What's the average salary of high-risk employees?"
        - "List top 5 employees by tenure"

        Args:
            message: User's question
            session_id: Session identifier
            employee_id: Optional selected employee HR code
            dataset_id: Optional dataset ID (uses active dataset if not provided)
            model: Optional LLM model override

        Returns:
            Dict with response, tool_history, and metadata
        """
        print(f"[CHAT_TOOLS] === TOOL-CALLING CHAT START ===", flush=True)
        print(f"[CHAT_TOOLS] message: '{message}', employee_id: {employee_id}", flush=True)
        ...
        print(f"[CHAT_TOOLS] Registered tools: {len(tool_registry)}", flush=True)

        # Build employee context if employee is selected
        employee_context = None
        if employee_id:
            employee = await self._get_employee_data(employee_id, dataset_id)
            if employee:
                employee_context = {
                    "hr_code": employee.hr_code,
                    "full_name": employee.full_name,
        ...
        try:
            result = await agent.run(
                ...
                system_prompt=self._get_tool_calling_system_prompt()
            )

            print(f"[CHAT_TOOLS] Agent completed - iterations: {result.get('iterations')}, "
                  f"tools_called: {len(result.get('tool_history', []))}", flush=True)

            # Save messages
            await self._save_message(session_id, employee_id, message, "user")
            await self._save_message(session_id, employee_id, result.get("response", ""), "assistant")
            ...
            return {
                ...
                "pattern_detected": "tool_calling",
                "structured_data": None,
                "success": result.get("success", True)
            }

        except Exception as e:
            print(f"[CHAT_TOOLS] ERROR: {e}", flush=True)
            import traceback
            traceback.print_exc()

            return {
                "response": f"I encountered an error while analyzing your question: {str(e)}",
                "tool_history": [],
                "iterations": 0,
                "tokens_used": 0,
            ...
            print(f"[CHAT_TOOLS] === TOOL-CALLING CHAT END ===", flush=True)
    ...
    async def stream_chat_with_tools(
        ...
        """
        Streaming version of chat_with_tools for WebSocket.
        ...
        """
        print(f"[CHAT_TOOLS_STREAM] === STREAMING TOOL-CALLING CHAT START ===", flush=True)
        ...
        # Save messages
        await self._save_message(session_id, employee_id, message, "user")
        await self._save_message(session_id, employee_id, full_response, "assistant")

        print(f"[CHAT_TOOLS_STREAM] === STREAMING TOOL-CALLING CHAT END ===", flush=True)

    def _get_tool_calling_system_prompt(self) -> str:
        """Get the system prompt for tool-calling mode"""
        return """You are a helpful AI assistant for ChurnVision Enterprise, an employee churn prediction platform.
You help users understand their workforce data by querying the actual database.

IMPORTANT RULES:
1. ALWAYS use tools to get real data - NEVER make up or estimate numbers
2. When asked about counts, averages, or statistics, use the appropriate analysis tools
3. After getting tool results, provide a clear, concise summary of the findings
4. If a tool returns an error, explain what went wrong and suggest alternatives
5. Be precise with numbers - include exact counts and percentages when available

You have access to tools that can:
- Count employees with various filters (department, tenure, salary, risk level)
- Calculate aggregates (average salary, total cost, min/max values)
- Look up individual employee information
- Query churn predictions and risk scores
- Analyze department statistics

When answering, be direct and data-driven. Include relevant numbers from the tool results."""
