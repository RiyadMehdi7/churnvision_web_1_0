...
from app.services.utils.json_helpers import parse_json_response, safe_json_loads
...
class TreatmentGenerationService(BaseAIGenerationService):
    """
...
"""
    ...
    def _construct_prompt(
        ...
        """Construct the main prompt for treatment generation."""
        ...
        # Determine urgency and treatment focus
        urgency, focus = self._get_urgency_and_focus(risk_score, dataset_id)
...
[
  {{
    ...
    def _parse_response(
        self,
        response_text: str,
        employee: Optional[HRDataInput] = None,
        churn_data: Optional[ChurnOutput] = None
    ) -> List[Dict[str, Any]]:
        """Parse treatment list from LLM response."""
        MIN_TREATMENTS = 3

        try:
            treatments = parse_json_response(response_text, expect_type="array")
            ...
            if len(valid_treatments) < MIN_TREATMENTS:
                logger.info(f"AI returned only {len(valid_treatments)} treatments, supplementing with fallback")
    ...
    def _generate_fallback_treatments(
        self,
        employee: Optional[HRDataInput] = None,
        churn_data: Optional[ChurnOutput] = None
    ) -> List[Dict[str, Any]]:
