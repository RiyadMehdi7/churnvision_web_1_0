...
from app.services.ml.dataset_profiler_service import DatasetProfile
...
class ModelRouterService:
    ...
    def _score_lightgbm(self, profile: DatasetProfile) -> float:
        ...
        if profile.n_categorical_features > 0:
            score = min(1.0, score + 0.05)
    ...
    def _score_catboost(self, profile: DatasetProfile) -> float:
        ...
        if profile.n_categorical_features > 3:
            score = min(1.0, score + 0.15)
        elif profile.n_categorical_features > 0:
            score = min(1.0, score + 0.08)
    ...
    def _get_model_reasoning(
        ...
        if model == "tabpfn":
            ...
            if profile.n_categorical_features > 0:
                reasons.append(
                    f"Categorical features ({profile.n_categorical_features}) "
