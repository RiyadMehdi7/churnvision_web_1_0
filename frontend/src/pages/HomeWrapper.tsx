...
export const HomeWrapper: React.FC = () => {
  ...
  // Check AI/LLM readiness via backend API (web application)
  useEffect(() => {
    const check = async () => {
      try {
        const response = await api.get('/ai/status');
        if (response.data && typeof response.data.ready === 'boolean') {
          setLlmReady(!!response.data.ready);
        }
      } catch (error) {
    ...
    // For web application, could implement polling or websockets for real-time updates
    // For now, just check once on mount
  }, []);
                ...
                onClick={async () => {
                  try {
                    ...
                    if (response.data?.success) {
                      // Recheck AI status after provisioning
                      const statusResponse = await api.get('/ai/status');
                      setLlmReady(!!statusResponse.data?.ready);
                    } else {
                      alert(`Provisioning failed: ${response.data?.message || 'Unknown error'}`);
                    }
                  } catch (e: any) {
                    alert(`Provisioning failed: ${e.response?.data?.detail || e?.message || e}`);
                  }
                }}
              >
                Provision Model
