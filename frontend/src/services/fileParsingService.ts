...
/**
 * Parse an Excel file
 * NOTE: Excel parsing is now handled by the FastAPI backend.
 * This function returns an error message directing users to upload via the API.
 * @param _file The Excel file to parse (unused - handled by backend)
 * @returns Promise with error message
 */
const parseExcel = async (_file: File): Promise<ParsedData> => {
  return Promise.resolve({
    data: [],
    columns: [],
    error: 'Excel file parsing is handled by the backend. Please use CSV format for client-side parsing, or upload directly to the API endpoint.'
  });
};
