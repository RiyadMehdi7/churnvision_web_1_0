...
export const TreatmentTracker: React.FC<TreatmentTrackerProps> = ({
  ...
  const loadTreatmentData = async () => {
    if (!hasDBConnection || !activeProject?.dbPath) return;
    
    setIsLoading(true);
    try {
      // Load employee treatments if selected
      if (selectedEmployee) {
        const apps = await treatmentTrackingService.getEmployeeApplications(activeProject.dbPath, selectedEmployee.hr_code);
        setEmployeeTreatments(apps);
      }
      
      // Load overall effectiveness data
      const effectiveness = await treatmentTrackingService.getEffectiveness(activeProject.dbPath);
      setTreatmentEffectiveness(effectiveness);
      
      // Load A/B test results
      const abTests = await treatmentTrackingService.getABTests(activeProject.dbPath);
      setAbTestResults(abTests);
      
      // Get last sync time
      const syncTime = await treatmentTrackingService.getLastSync(activeProject.dbPath);
      setLastSyncTime(syncTime);

      // Load uplift summary via IPC first, fallback to HTTP
      try {
        const ipcUplift = (window as any).electronApi?.uplift;
        if (ipcUplift?.summary) {
          const resp = await ipcUplift.summary();
          if (resp?.success) {
            setUpliftBasic(resp.results || []);
            setUpliftCuped(resp.cuped || []);
          }
        } else {
          const basic = await api.get('/api/eltv/uplift/basic', { params: { projectId: activeProject.dbPath } });
          setUpliftBasic(basic.data?.data || []);
          const tr = await api.get('/api/eltv/uplift/tr', { params: { projectId: activeProject.dbPath } });
          // Could display TR results; for now, keep CUPED empty in HTTP path
          setUpliftCuped([]);
        }
      } catch (e) {
  ...
  const syncWithHR = async () => {
    ...
    setIsLoading(true);
    try {
      const connectionDetails = (window as any)?.electronApi?.connections?.getActiveConnection?.() || {};
      const result = await treatmentTrackingService.syncHR(activeProject.dbPath, connectionDetails);
  ...
  const applyTreatmentWithTracking = async (treatment: any, abGroup: 'control' | 'treatment' = 'treatment') => {
    ...
    try {
      // Apply treatment through existing system
      const result = await window.electronApi?.applySelectedTreatment?.(
        selectedEmployee.hr_code, 
        treatment.id
      );
      
      // Record treatment application for tracking
      if (result && (window as any).electronApi?.treatment?.recordApplication) {
        try {
          ...
          await (window as any).electronApi.treatment.recordApplication({
  ...
  const createABTest = async () => {
    ...
    try {
      const created = await treatmentTrackingService.createABTest(activeProject.dbPath);
      if (created?.testName) {
        toast({
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              treatment.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            item.effectiveness_rate > 0.7 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                        ...
                        {results.map(result => (
                          <div
                            ...
                            className={`p-3 rounded border ${
                              result.group_assignment === 'treatment'
                          ...
                          <div className="text-sm">
                            ...
                            {(() => {
                              ...
                              if (treatment && control) {
                                ...
                                return (
                                  <span className={`font-medium ${
                                    effect > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                                  }`}>
