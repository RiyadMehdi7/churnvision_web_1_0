  ...
  return {
    ...
    fetchTrainingStatus: async (projectId) => {
      if (!projectId) {
        ...
        // Optionally set status to idle if no project
        // set({ trainingStatus: { status: 'idle', ..., companyId: '' } });
        return;
      }
      try {
        const electronApi = (window as any).electronApi;
        if (!electronApi?.getModelTrainingStatus) {
          console.error('[DataCache] getModelTrainingStatus API not available.');
          return;
        }
        // Pass projectId if the backend API requires it
        const result = await electronApi.getModelTrainingStatus(/* projectId */);
        if (!result.success || !result.status) {
          console.error(`[DataCache] Failed to get training status for projectId: ${projectId}`);
          // Optionally set error status
          // set({ trainingStatus: { status: 'error', message: 'Failed to fetch status', ... } });
          return;
        }

        const currentStatus = get().trainingStatus;
        const newStatus = result.status;
        const justCompleted = newStatus.status === 'complete' && currentStatus?.status !== 'complete';
        ...
        if (justCompleted) {
          ...
          // Pass projectId to fetchHomeData
  ...
  try {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        const reasoningData = await reasoningService.getBatchReasoning(batch);
