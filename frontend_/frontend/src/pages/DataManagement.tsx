...
export function DataManagement(): React.ReactElement {
    ...
    const fetchModelMetrics = useCallback(async () => {
        ...
        try {
            ...
            const result = await window.electronApi?.getModelMetrics?.();
            if (result?.success) {
    ...
    // Helper to check if the Electron API is available
    const getElectronApi = useCallback(() => {
        const api = (window as any).electronApi;
        if (!api) {
            const errorMsg = 'Electron API is not available. Ensure preload script is loaded.';
            logger.error('DataManagement: ' + errorMsg, undefined, 'DataManagement');
            setGeneralError(errorMsg);
            // Disable functionalities that depend on the API
            setIsLoading(false);
            // Use context loading state
            // setIsProjectLoading(false); // Replaced by context
    ...
    useEffect(() => {
        ...
        if (api) {
            ...
            setProjectError('Electron API not available for fetching projects.');
            setIsProjectListLoading(false);
            setProjects([]);
        }
        // REMOVED: activeProject fetching (handled by context)
        // REMOVED: active project listener setup (handled by context)
    }, [getElectronApi]); // Removed dependencies related to local active project
    ...
    const fetchProjects = useCallback(async () => {
        ...
        if (!api?.projects?.list) {
            setProjectError('Project API (list) not available.');
            setIsProjectListLoading(false);
            return;
        }
    ...
    const handleSetActiveProject = useCallback(async (dbPath: string | null) => {
        const api = getElectronApi();
        if (!api?.projects?.setActive) {
            setProjectError('Project API (setActive) not available.');
            return;
        }
        setProjectError(null);
        // Optional: Set a temporary loading state if needed
        try {
            const result = await api.projects.setActive(dbPath);
            if (!result.success) {
    ...
    const handleCreateProject = useCallback(async () => {
        ...
        if (!api?.projects?.create) {
            setProjectError('Project API (create) not available.');
            return;
        }
        setIsCreatingProject(true);
        setProjectError(null);
        try {
            const result = await api.projects.create(newProjectName.trim());
            if (result.success) {
    ...
    }, [newProjectName, fetchProjects, getElectronApi, handleSetActiveProject]);
    ...
    const handleDeleteProject = useCallback(async (project: Project) => {
        ...
        const api = getElectronApi();
        if (!api?.projects?.delete) {
            setProjectError('Project API (delete) not available.');
            return;
        }
        ...
        try {
            const result = await api.projects.delete(project.path);
            if (result.success) {
    ...
    const handleExportProject = useCallback(async () => {
        const api = getElectronApi();
        if (!activeProject || !api?.exportProject) {
            setProjectError("Active project is required to export, or API is unavailable.");
            // Optionally, use toast here
            // toast({ title: "Export Error", description: "Active project required or API unavailable.", variant: "destructive" });
            return;
        }
        setIsProjectActionLoading(true);
        setProjectError(null);
        try {
            const result = await api.exportProject(activeProject.name, activeProject.path); // CHANGED from activeProject.dbPath
            if (result.success) {
    ...
    }, [activeProject, getElectronApi]);

    const handleImportProject = useCallback(async () => {
        const api = getElectronApi();
        if (!api?.importProject) {
            setProjectError("Project import API not available.");
            // toast({ title: "Import Error", description: "Import function unavailable.", variant: "destructive" });
            return;
        }
        setIsProjectActionLoading(true);
        setProjectError(null);
    ...
    const fetchDatasets = useCallback(async () => {
        ...
        if (!electronApi?.listDatasets) {
            logger.error("Error fetching datasets: datasets.list API not available.", undefined, 'DataManagement');
            setDatasets([]);
            setGeneralError('Dataset API (list) unavailable.');
            setIsLoading(false); // Update general loading state
            return;
        }
        ...
        try {
            setGeneralError('');
            logger.info('Fetching datasets via IPC for context project...', { activeProject: activeProject.name }, 'DataManagement');
            const fetchedDatasetsRaw = await electronApi.listDatasets();
            logger.info('Datasets received via IPC', { count: fetchedDatasetsRaw.length }, 'DataManagement');

            const mappedDatasets: Dataset[] = fetchedDatasetsRaw.map((raw: any) => ({
                id: raw.dataset_id,
                name: raw.name,
                type: raw.file_type || 'Unknown',
                size: raw.size || 0,
                uploadedAt: raw.upload_date,
                rowCount: raw.row_count,
                active: raw.is_active === 1 || raw.is_active === true,
                isSnapshot: raw.is_snapshot === 1 || raw.is_snapshot === true,
                snapshotGroup: raw.snapshot_group || null,
                snapshotPairDatasetId: raw.snapshot_pair_dataset_id || null,
            ...
            logger.error('Error fetching datasets via IPC:', error, 'DataManagement');
            setDatasets([]);
            setGeneralError(error.message || 'Failed to fetch datasets via IPC.');
        } finally {
            setIsLoading(false); // Update general loading state
        }
        // Add dependencies carefully. activeProject comes from context.
        // fetchDatasets itself is stable due to useCallback.
    }, [activeProject]);
    ...
    const handleDeleteDataset = useCallback(async (datasetId: string) => {
        ...
        const electronApi = window.electronApi;
        if (!electronApi?.deleteDataset) {
            logger.error("Error deleting dataset: deleteDataset API not available.", undefined, 'DataManagement');
            setGeneralError('Dataset API (delete) unavailable.');
            return;
        }
        try {
            logger.info(`Requesting dataset deletion via IPC: ${datasetId}`, undefined, 'DataManagement');
            const result = await electronApi.deleteDataset(datasetId);
            if (result?.success) {
                logger.info(`Dataset ${datasetId} deleted successfully via IPC. Refreshing list.`, { cancelled: result.cancelled }, 'DataManagement');
                fetchDatasets(); // Refresh list (will use current context project)
            } else {
                throw new Error(result?.message || 'Failed to delete dataset via IPC');
            }
        } catch (error: any) {
            logger.error(`Error deleting dataset ${datasetId} via IPC:`, error, 'DataManagement');
    ...
    const fetchConnections = useCallback(async () => {
        ...
        const electronApi = window.electronApi;
        if (!electronApi?.listConnections) {
            logger.error("Error fetching connections: listConnections API not available.", undefined, 'DataManagement');
            setSelectedConnection(null);
            // Consider setting general error and loading state
            return;
        }
        setIsLoading(true); // Use general loading state
        try {
            logger.info('Fetching connections via IPC for context project...', { activeProject: activeProject.name }, 'DataManagement');
            const fetchedConnections = await electronApi.listConnections();
            logger.info(`Connections received via IPC: ${fetchedConnections.length}`, undefined, 'DataManagement');
            const mappedConnections: Connection[] = fetchedConnections.map((shortConn: any) => ({ // Add type safety if possible
                id: shortConn.connection_id,
                name: shortConn.name,
                type: shortConn.type as DbType,
                host: shortConn.host,
                lastConnected: '', // Add default or map if available
                status: 'inactive' // Add default or map if available
            }));
            ...
            logger.error('Error fetching connections via IPC:', error, 'DataManagement');
            setConnections([]);
            setSelectedConnection(null);
    ...
    const handleListTables = useCallback(async (connectionId: string) => {
        ...
        const electronApi = window.electronApi;
        if (!electronApi?.listTables) {
            logger.error("Error listing tables: listTables API not available.", undefined, 'DataManagement');
        ...
        try {
            ...
            const tables = await electronApi.listTables(connectionId);
    ...
    const handleImportData = useCallback(async () => {
        ...
        const electronApi = window.electronApi;
        if (!electronApi?.importFromDb) {
            logger.error("Error importing data: importFromDb API not available.", undefined, 'DataManagement');
            setGeneralError('API unavailable.');
            return;
        }
        ...
        try {
            ...
            const result = await electronApi.importFromDb({
    ...
    const handleSaveConnection = useCallback(async () => {
        ...
        const electronApi = window.electronApi;
        if (!electronApi?.createConnection) {
            logger.error("Error creating connection: createConnection API not available.", undefined, 'DataManagement');
            setConnectionMessage('Connection API (create) unavailable.');
            setConnectionStatus('error'); // Added to show error state visually
            return;
        }
        try {
            logger.info(`Requesting connection creation via IPC: ${dbConfig.host}:${dbConfig.port}`, undefined, 'DataManagement');
            const connectionName = `${dbType} @ ${dbConfig.host}`; // Generate name
            const result = await electronApi.createConnection({
            ...
            if (result?.success) {
                logger.info(`Connection created successfully via IPC. Refreshing list.`, undefined, 'DataManagement');
                fetchConnections(); // Refresh list (will use current context project)
                setConnectionMessage('Connection created successfully.');
            } else {
                throw new Error(result?.message || 'Failed to create connection via IPC');
            }
        } catch (error: any) {
            logger.error(`Error creating connection via IPC:`, error, 'DataManagement');
    ...
    const uploadData = useCallback(async (): Promise<void> => {
        ...
        const electronApi = window.electronApi; // Use window.electronApi directly
        if (!electronApi?.uploadFile) { // Check for uploadFile
            logger.error("Error uploading data: uploadFile API not available.", undefined, 'DataManagement');
            setUploadStatus('error');
            setUploadMessage('File upload API unavailable.');
            setGeneralError('File upload API unavailable.');
            return;
        }
        ...
        try {
            ...
            // NEW IPC CALL:
            // Pass projectId to uploadFile if the backend needs it
            // Assuming uploadFile implicitly uses the active project set in main process
            const result = await electronApi.uploadFile({
                fileDataBuffer: fileDataBuffer,
                mimeType: fixedPreview ? 'text/csv' : selectedFile.type,
                originalFilename: fixedPreview ? `autofixed_${selectedFile.name.replace(/\.(csv|xlsx|xls)$/i, '')}.csv` : selectedFile.name,
                mappings: backendMappings,
                datasetName: currentDatasetName.trim(),
                size: fileSize,
                xDataMode: (localStorage.getItem('settings.dataMode') === 'performance') ? 'performance' : 'wage'
                // projectId: currentProjectId // Add if needed by backend
            });
    ...
    // Modified test connection handler to use real API
    const handleTestConnection = useCallback(async () => {
        setConnectionStatus('testing');
        setConnectionMessage('');
        const electronApi = window.electronApi; // Use window.electronApi directly
        if (!electronApi?.testConnection) { // Use testConnection
            logger.error("Error testing connection: testConnection API not available.", undefined, 'DataManagement');
            setConnectionStatus('error');
            setConnectionMessage('Connection API (test) unavailable.');
            return;
        }
        ...
        try {
            ...
            const params: ConnectionTestParams = {
                type: dbType, // Assuming dbType state holds the type string
                host: dbConfig.host,
                port: dbConfig.port, // Pass as string or number as needed
                username: dbConfig.username,
                password: dbConfig.password, // Optional
                databaseName: dbConfig.databaseName
            };

            logger.info('Testing connection via IPC...', { params }, 'DataManagement');
            const result = await electronApi.testConnection(params); // Use testConnection with params
    ...
    // Update setActiveDataset to use activeProject from context
    const setActiveDataset = useCallback(async (datasetId: string) => {
        ...
        if (!electronApi?.activateDataset) {
            logger.error("Error activating dataset: activateDataset API not available.", undefined, 'DataManagement');
            setGeneralError('Dataset API unavailable.');
            return;
        }

        setGeneralError('');
        try {
            logger.info('Setting active dataset via IPC...', { datasetId }, 'DataManagement');
            const result = await electronApi.activateDataset(datasetId);
    ...
    const handleStartTraining = useCallback(async () => {
        ...
        const electronApi = (window as any).electronApi;
        if (!electronApi?.trainModel) {
            logger.error("Error starting training: trainModel API not available.", undefined, 'DataManagement');
            setGeneralError('Training API unavailable.');
            return;
        }
        ...
        try {
            // Pass projectId if needed by backend API
            const result = await electronApi.trainModel(/* { projectId: currentProjectId } */);
            if (result.success && result.status) {
                logger.info('Training successfully queued via IPC.', { status: result.status }, 'DataManagement');
            ...
            logger.error('Error starting training via IPC:', error, 'DataManagement');
            setGeneralError(error.message || 'Could not start training via IPC.');
        }
