  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-5 flex flex-col">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <div className={`flex-shrink-0 w-3 h-3 rounded-full ${
              isSelected ? 'bg-blue-500 shadow-lg shadow-blue-500/50' : 'bg-gray-300 dark:bg-gray-600'
        ...
        {/* Action Button */}
        <button 
          className={`mt-auto w-full py-3 px-4 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 ${
            isLoading
...
export function Playground() {
  ...
  useEffect(() => {
    const checkProjectData = async () => {
      try {
        const result = await window.electronApi?.checkProjectData?.();
        setHasDBConnection(Boolean(result?.hasData));
      } catch (_error) {
  ...
  const applyTreatmentToCandidate = useCallback(async (
    ...
    if (!window.electronApi?.applySelectedTreatment) {
      if (!silent) {
        toast({
          title: 'Service Unavailable',
          description: 'Treatment service is not available. Please restart the application.',
          variant: 'destructive',
        });
      }
      return null;
  ...
  const applyBulkTreatment = useCallback(async () => {
    ...
    if (!window.electronApi?.applySelectedTreatment) {
      toast({
        title: 'Service Unavailable',
        description: 'Treatment application service is not available. Please restart the application.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const applyTreatment = useCallback(async (treatment: TreatmentSuggestion) => {
    if (!selectedEmployee || !window.electronApi?.applySelectedTreatment) {
      return;
    }
  ...
  const handleEmployeeSelect = useCallback(async (employeeId: number, treatmentIdToSelect?: number) => {
      ...
      if (!window.electronApi?.getPlaygroundData) {
          setError("Electron API not available. Unable to fetch playground data.");
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
              const [playgroundData, suggestionsResult] = await Promise.all([
                  window.electronApi.getPlaygroundData(employee.hr_code),
                  window.electronApi.getTreatmentSuggestions(employee.hr_code)
              ]);
...
const ScenarioComparisonTab = memo(({
  ...
  const updateScenarioTreatment = useCallback(async (scenarioId: string, treatmentId: number | null) => {
    ...
    if (!window.electronApi?.applySelectedTreatment) return;
...
const MassTreatmentTab = memo(({
    ...
    try {
      ...
      const candidatePromises = employeeScores.map(async (scoredEmployee) => {
        try {
          ...
          if (!window.electronApi?.getPlaygroundData || !window.electronApi?.getTreatmentSuggestions) {
            return null;
          }
  ...
  const applyWhatIf = useCallback(async (employee: Employee) => {
    if (!window.electronApi?.manualSimulate) return;
    try {
      ...
      const resp = await window.electronApi.manualSimulate({ employeeId: employee.hr_code, changedFeatures });
