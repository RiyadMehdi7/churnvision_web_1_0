...
import providerCatalog from '@shared/providerCatalog.json';
...
export function Settings() {
  ...
  const refreshProviderStatus = useCallback(async () => {
    try {
      const status = await (window as any)?.electronApi?.ai?.getProviderStatus?.();
      if (status && mountedRef.current) {
        setProviderStatus(status as Record<string, ProviderInstallationStatus>);
      }
    } catch (error) {
  ...
  const loadProviderPreference = useCallback(async () => {
    try {
      ...
      const modelType = response?.modelType;
  ...
  const loadStrictOfflineState = useCallback(async () => {
    ...
    try {
      const status = await (window as any)?.electronApi?.security?.getStrictOfflineMode?.();
      if (mountedRef.current) {
        setStrictOffline(!!status?.enabled);
      }
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = (window as any)?.electronApi?.ai?.onProviderProvisionProgress?.((payload: ProviderInstallationStatus & { providerId?: string }) => {
      ...
      if (payload.status === 'completed') {
        if (mountedRef.current) {
          setPendingProvision(prev => ({ ...prev, [payload.providerId!]: false }));
        }
        refreshProviderStatus();
        toast({
          title: `${providerLabelLookup[payload.providerId!] ?? payload.providerId} ready`,
          description: 'Model provisioned successfully.',
        });
      }
      ...
      if (payload.status === 'error') {
        ...
        toast({
          ...
          description: payload.error || 'An error occurred while downloading the model.',
          variant: 'destructive',
        });
      }
    });
  ...
  const ensureProviderProvisioned = useCallback(async (option: ProviderOption) => {
    ...
    if (!(window as any)?.electronApi?.ai?.provisionProvider) {
      throw new Error('Provisioning channel unavailable.');
    }

    setPendingProvision(prev => ({ ...prev, [option.id]: true }));

    try {
      const result = await (window as any).electronApi.ai.provisionProvider({ providerId: option.id });
      if (!result?.success) {
  ...
  const handleProviderSelect = useCallback(
    async (option: ProviderOption, optionDisabled: boolean) => {
      ...
      try {
        if (typeof window !== 'undefined' && (window as any).electronApi?.ai?.setModelType) {
          await (window as any).electronApi.ai.setModelType(nextProvider);
        }
...
const manifestProviders = (providerCatalog as ProviderCatalogManifest).providers;
...
const SecuritySettings: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      const s = await (window as any).electronApi?.security?.getStrictOfflineMode?.();
      setStrictOffline(!!s?.enabled);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const toggleStrict = useCallback(async (enabled: boolean) => {
    setStrictOffline(enabled);
    try {
      await (window as any).electronApi?.security?.setStrictOfflineMode?.(enabled);
      if (typeof window !== 'undefined') {
        ...
        <CardContent className="space-y-6">
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  if (!api?.provisionFromModelfile) throw new Error('Provisioning not available');
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">Save an encrypted copy of the active project database</div>
            </div>
            <Button
              variant="outline"
              onClick={async () => {
                try {
                  const res = await (window as any).electronApi?.projects?.exportEncryptedBackup?.();
                  if (res?.success) alert(`Encrypted backup saved to: ${res.path}`);
                  else if (!res?.cancelled) alert(`Backup failed: ${res?.error || 'Unknown error'}`);
                } catch (e) {
                  const msg = (e && (e as any).message) ? (e as any).message : String(e);
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">Add internal docs (txt/markdown) for on-device RAG</div>
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="secondary"
                onClick={async () => {
                  try {
                    const api = (window as any).electronApi?.rag;
                    const paths = await api.openFileDialog();
                    if (!paths || paths.length === 0) return;
                    const res = await api.ingestFiles(paths);
                    if (res?.success) alert(`Ingested ${res.ingested} document(s).`);
                    else alert(`Ingestion failed: ${res?.error || 'Unknown error'}`);
                  } catch (e) {
                    const msg = (e && (e as any).message) ? (e as any).message : String(e);
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (stats?.success) alert(`Docs: ${stats.documents}, Chunks: ${stats.chunks}`);
                }}
              >
                Stats
              </Button>
