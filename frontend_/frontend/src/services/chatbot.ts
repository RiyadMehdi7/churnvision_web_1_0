...
class ChatbotService {
  ...
  private async handleOnline(): Promise<void> {
    ...
    if (!window.electronApi || !window.electronApi.chatbot) {
      // Electron IPC bridge not available. Cannot process offline queue
      return;
    }
    ...
    while (this.offlineQueue.length > 0) {
      ...
      if (item) {
        try {
            // Sending queued message silently in production
            // Send using IPC
            ...
            // Error sending offline message via IPC silently in production
  ...
  async sendMessage(payload: SendMessagePayload): Promise<ChatResponse> {
    // --- BEGIN SIMULATION --- 
    // Check if the message is a similarity analysis request
    // const similarityRegex = /^Compare (.*) with similar resigned employees\.?$/i;
    // const match = payload.content.match(similarityRegex);
    // 
    // if (match && match[1]) {
    //   const targetEmployeeName = match[1].trim();
    //   console.log(`[Simulation] Detected similarity request for: ${targetEmployeeName}`);
    // 
    //   // Construct the simulated structured response
    //   const simulatedData = {
    //     type: 'similarityAnalysis',
    //     targetEmployeeName: targetEmployeeName,
    //     similarEmployees: [
    //       { name: 'Alex Johnson (Resigned)', department: 'Engineering', similarityScore: 0.82 },
    //       { name: 'Samantha Lee (Resigned)', department: 'Sales', similarityScore: 0.75 },
    //       { name: 'David Chen (Resigned)', department: 'Engineering', similarityScore: 0.68 },
    //     ],
    //     explanation: `Based on factors like tenure, performance ratings, and project involvement, ${targetEmployeeName} shares several key characteristics with previously departed employees Alex Johnson, Samantha Lee, and David Chen. Notably, Alex Johnson from the same department exhibited a similar decline in engagement metrics prior to departure. Samantha Lee, while in Sales, had a comparable sentiment analysis trend in recent reviews.`
    //   };
    // 
    //   // Return a ChatResponse object with the JSON string
    //   return {
    //     response: { 
    //       message: JSON.stringify(simulatedData), // Send JSON string
    //       responseTime: 50 // Dummy response time
    //     },
    //     botMessageId: `sim-${uuidv4()}`,
    //     success: true,
    //     userMessageId: `user-${uuidv4()}` // Add a placeholder user ID if needed
    //   };
    // }
    // --- END SIMULATION ---
    ...
    if (!window.electronApi || !window.electronApi.chatbot) {
      // Electron IPC bridge not available - logged silently in production
      ...
      // If online but bridge missing, it's a critical error
      throw new Error('Chat service communication channel is unavailable.');
    }

    try {
      // Sending message via IPC silently in production
      // Use the exposed IPC handler from preload script
      const response = await window.electronApi.chatbot.sendMessage(payload);
      
      if (!response || !response.response) {
         // Log unexpected response structure
         // Invalid response structure received from IPC handler silently in production
         throw new Error('Received invalid response structure from chatbot service.');
      }
      
      // TODO: Update analytics based on response
      
      return response; // Return the response directly from IPC handler
    } catch (error: any) {
      // Chatbot IPC Error (sendMessage) - logged silently in production
      // Rethrow a user-friendly error, potentially parsing error details if available
  ...
  async getChatHistory(sessionId: string): Promise<ChatMessage[]> {
    ...
    if (!window.electronApi || !window.electronApi.chatbot) {
      // Electron IPC bridge not available - logged silently in production
      // Cannot fetch history without the bridge
      throw new Error('Chat service communication channel is unavailable.');
    }

    if (!sessionId) {
        // getChatHistory called without a sessionId - logged silently in production
        return []; // Return empty if no session ID provided
    }
    
    try {
      // Fetching chat history via IPC silently in production
      // Use the exposed IPC handler from preload script
      const history = await window.electronApi.chatbot.getHistory(sessionId);
      // Ensure history is an array, default to empty array if not
      return Array.isArray(history) ? history : []; 
    } catch (error: any) {
      // Log the specific error from IPC
      // Chat History IPC Error - logged silently in production
      // Check if it's a specific "not found" error if the main process signals it
      // if (error.message?.includes('History not found')) { return []; } 
      
      // Rethrow a user-friendly error
      // The catch block in AIAssistant.tsx might handle this now based on IPC errors
      throw new Error(error.message || 'Failed to fetch chat history via chat service.');
    }
  }
