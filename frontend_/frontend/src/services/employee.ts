...
class EmployeeService {
  ...
  private async fetchEmployees(projectId: string | null, forceRefresh = false): Promise<ApiEmployee[]> {
    ...
    if (!projectId && !this.isOfflineMode) {
        console.warn('[fetchEmployees] No projectId provided for online fetch. Returning empty array.');
        return [];
    }
    ...
    try {
      console.log(`Fetching employee data via IPC for project ${projectId} with forceRefresh: ${forceRefresh}`);
      ...
      if (window.electronApi?.listEmployees) {
        rawResponse = await window.electronApi.listEmployees({
      ...
      } else if (window.electronApi?.getCombinedData) {
        rawResponse = await window.electronApi.getCombinedData(projectId);
      } else {
        console.error('Electron API for employee listing is not available.');
        return this.getFromCache(projectId) || [];
      }

      const responseRows = Array.isArray(rawResponse?.rows)
        ? rawResponse.rows
        : Array.isArray(rawResponse)
          ? rawResponse
          : [];

      console.log('Successfully fetched employee data via IPC for project:', projectId, { count: responseRows.length });
      ...
      if (typedEmployees.length > 0) {
        ...
        console.log(`Processed ${normalizedEmployees.length} employees via IPC for project ${projectId}`);
        
        // Save to cache
        this.saveToCache(projectId, normalizedEmployees);
        return normalizedEmployees;
      } else {
         console.warn(`No employees found in the IPC response for project ${projectId}`);
