...
export const treatmentTrackingService = {
  async getEmployeeApplications(projectId: string, employeeId: string): Promise<TreatmentApplication[]> {
    // IPC-first fallback to avoid HTTP dependency
    const ipc = (window as any).electronApi?.treatment;
    if (ipc?.getEmployeeApplications) {
      try { return await ipc.getEmployeeApplications(employeeId); } catch {}
    }
  ...
  async getEffectiveness(projectId: string): Promise<TreatmentEffectiveness[]> {
    const ipc = (window as any).electronApi?.treatment;
    if (ipc?.getEffectiveness) { try { return await ipc.getEffectiveness(); } catch {} }
  ...
  async getABTests(projectId: string, testName?: string): Promise<ABTestResult[]> {
    const ipc = (window as any).electronApi?.treatment;
    if (ipc?.getABTests) { try { return await ipc.getABTests(testName); } catch {} }
  ...
  async getLastSync(projectId: string): Promise<string | null> {
    const ipc = (window as any).electronApi?.treatment;
    if (ipc?.getLastSync) { try { return await ipc.getLastSync(); } catch {} }
