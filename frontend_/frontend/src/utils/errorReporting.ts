...
class ErrorReportingService {
  ...
  private sendToElectron(report: ErrorReport): void {
    try {
      if (window.electronApi?.logger?.error) {
        window.electronApi.logger.error('Frontend Error Report', {
          errorReport: report,
        });
      }
    } catch (error) {
      console.warn('Failed to send error report to Electron:', error);
    }
  }
